...
def get_args():
    ...
    parser.add_argument('--memory_capacity', type=int, default=10000, help='ReplayBuffer size')
...
class PrioritizedBuffer(object):
    def __init__(self, opt, num_states):
        ...
        self.state = np.zeros((max_size, 4, num_states, num_states))
        self.action = np.zeros((max_size, 1))
        self.reward = np.zeros((max_size, 1))
        self.next_state = np.zeros((max_size, 4, num_states, num_states))
...
class DQN:
    def __init__(self, GAMMA, LR, EPSILON, num_actions, num_states, batch_size, target_update_steps):
        self.eval_net, self.target_net = Net(num_actions).to(device), Net(num_actions).to(device)
        self.learn_count = 0
        self.curr_step = 0
        self.lr = LR
...
def eval_model(model_path, opt, p):
    ...
    with torch.no_grad():
        for _ in range(5):
            p.reset_game()
            obs = convert(p.getScreenGrayscale())
            ...
            while True:
                ...
                state_ = np.append(state.cpu().numpy()[:, 1:, ...], obs, axis=1)
                done = p.game_over()
                if done:
                    break
                step += 1
...
def train(opt):
    ...
    for i_episode in range(1, opt.max_episodes, 1):
        ...
        while True:
            ...
            obs = convert(p.getScreenGrayscale())
            obs = np.reshape(obs, [1, 1, obs.shape[0], obs.shape[1]])
            state_ = np.append(state[:, 1:, ...], obs, axis=1)
            done = p.game_over()
            sum_reward += reward

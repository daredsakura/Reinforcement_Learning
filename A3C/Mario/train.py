...
from torch.multiprocessing import Process, Queue  # pytorch 多进程
import shutil
...
def main(opt):
    ...
    train(0, opt, global_model, optimizer, True)
    # processes = []
    # for index in range(opt.num_processes):
    #     if index == 0:
    #         process = Process(target=train, args=(index, opt, global_model, optimizer, True))
    #     else:
    #         process = Process(target=train, args=(index, opt, global_model, optimizer))
    #     process.start()
